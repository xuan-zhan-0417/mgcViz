...
#' @export
#'
plot.nested1D <- function(x, inner = FALSE, smooth = FALSE, n = 100, xlim = NULL, ylim = NULL,
                          maxpo = 1e4, trans = identity, ci = TRUE, a_in = FALSE, ...) {

  if (inner) {
    # 1) Prepare data
    P <- .prepareInnerNested(o = x, n = n, xlim = xlim, ylim = ylim, smooth = smooth,
                             a_in, ...)

    out <- .plot.inner.nested.smooth.1D(P = P, trans = trans, maxpo = maxpo, ci = ci)
...
.plot.inner.nested.smooth.1D <- function(P, trans, maxpo, ci) {
  ...
  if (P$type == "si_nexpsm_coef") {
    ...
    .dat$fit <- data.frame("x" = P$x, "y" = P$fit, "ty" = trans(P$fit), "se" = P$se,
                           ...
                           "ty_lower" = trans(P$lower), "ty_upper" = trans(P$upper))
    .dat$misc <- list("trans" = trans)

    .pl <- ggplot(data = .dat$fit, aes(x = x, y = ty))
    .pl <- .pl +
      geom_errorbar(aes(ymin = ty_lower, ymax = ty_upper), linetype = "dashed")
    ...
    out <- list("ggObj" = .pl, "data"  = .dat, "type"  = c("si", "Factor"))
    ...
    out <- list("ggObj" = .pl, "data" = .dat, "type" = "1D")
  ...
  if (P$type == "mgks") {
    .dat <- list()
    .dat$fit <- data.frame("z" = drop(P$fit), "tz" = drop(trans(P$fit)),
                           "x" = rep(P$x, length(P$fit) / length(P$x)),
                           "y" = rep(P$y, each = length(P$fit) / length(P$x)), "se" = P$se)
  ...
  if (P$type == "si") {
    .dat <- list()
    .dat$fit <- data.frame("x" = as.factor(P$x), "y" = unname(P$fit),
                           "ty" = trans(unname(P$fit)), "se" = unname(P$se))
    .dat$misc <- list("trans" = trans)
